...
import { Grid3X3, LayoutGrid, Tv } from 'lucide-react';
...
function ShowsContent() {
  ...
  const getFiltersFromURL = useCallback((): ContentFilters => {
    const filters: ContentFilters = { type: 'show' };
  ...
  const handleFilterChange = useCallback((newFilters: ContentFilters) => {
    updateFilters({ ...newFilters, type: 'show', page: 1 });
  }, [updateFilters]);
  ...
  return (
    <div className="min-h-screen pt-20 pb-12">
      <div className="container mx-auto px-4">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl md:text-4xl font-bold mb-2">TV Shows</h1>
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            {data && (
              <p className="text-muted-foreground">
                {data.total.toLocaleString()} shows
                {filters.search && (
        ...
        {error ? (
          <div className="text-center py-20">
            <p className="text-accent mb-2">Failed to load shows</p>
